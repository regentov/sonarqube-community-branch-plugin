...
public class AzureDevOpsPullRequestDecorator implements PullRequestBuildStatusDecorator {
    ...
    public DecorationResult decorateQualityGateStatus(AnalysisDetails analysisDetails, AlmSettingDto almSettingDto, ProjectAlmSettingDto projectAlmSettingDto) {
        ...
        int pullRequestId;
        try {
            pullRequestId = Integer.parseInt(analysisDetails.getBranchName());
    ...
    private static PullRequest getPullRequest(AzureDevopsClient client, String projectId, String repositoryName, int pullRequestId) {
        try {
        ...
        try {
            GitPullRequestStatus gitPullRequestStatus = new GitPullRequestStatus(
                ...
                String.format("%s/dashboard?id=%s&pullRequest=%s",
                        ...
                        URLEncoder.encode(analysisDetails.getAnalysisProjectKey(), StandardCharsets.UTF_8.name()),
                        URLEncoder.encode(analysisDetails.getBranchName(), StandardCharsets.UTF_8.name())
                )
            );
